...
class toPhoto extends toDefault {
    ...
    public function _getMarkup() {
        ...
        if ($this->attributes[self::ATTR_SIZE] === 'Small')
            $return = sprintf("<img width='50px' src='/photo/%s/%s' class='%s'", $this->attributes[self::ATTR_USER_ID], $this->attributes[self::ATTR_SIZE], $this->attributes[self::ATTR_CLASS]);
        else {
            $return = sprintf("<img src='/photo/%s/%s' class='%s'", $this->attributes[self::ATTR_USER_ID], $this->attributes[self::ATTR_SIZE], $this->attributes[self::ATTR_CLASS]);
        }
