PhotoUploader = function( ){
  ...
  var markup = '<div class="PhotoUploader_FormContainer">'
             + '<a href="#" class="PhotoUploader_Close">x</a>'
             + '<form method="post" enctype="multipart/form-data" action="/photoManager/json/upload">'
             + '<h2>Choose a new photo to upload</h2>'
             + '<input type="file" name="photo">'
             + '<div class="Thumbnail" />'
             + '<button class="standard Blue">Upload</button>'
             + '</form>'
             + '</div>';

  this.start = function( )
  {
    $container = $('<div class="PhotoUploader" />').appendTo('body');
    ...
    iframe.get(0).onload = function( ){finished(iframe);};
